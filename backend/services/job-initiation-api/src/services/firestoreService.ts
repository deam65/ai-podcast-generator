...
export class FirestoreService {
  ...
  async createJob(job: Job): Promise<Job> {
    try {
      ...
      logger.error("Failed to create job in Firestore", {
        jobId: job.id,
        error,
      });
  ...
  async updateJobStatus(jobId: string, status: string): Promise<void> {
    try {
      await this.firestore.collection(this.collectionName).doc(jobId).update({
        status: status,
      ...
      logger.error("Failed to update job status in Firestore", {
        jobId,
        status,
        error,
      });
